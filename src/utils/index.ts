...
type CoffeeConsumptionHistory = {
  [timestamp: string]: CoffeeHistoryEntry;
};
...
// Define the `statusLevels` object with specific types
export const statusLevels: Record<string, StatusLevel> = {
  low: {
    color: "#047857",
    background: "#d1fae5",
    description:
...
// Define the coffeeConsumptionHistory with appropriate types
export const coffeeConsumptionHistory: CoffeeConsumptionHistory = {
...
// Define the coffeeOptions with the appropriate types
...
// Half-life constant in hours
const halfLifeHours: number = 5;
...
export function calculateCurrentCaffeineLevel(
  historyData: CoffeeConsumptionHistory
): string {
...
export function getTopThreeCoffees(historyData: CoffeeConsumptionHistory) {
  const coffeeCount: { [coffeeName: string]: number } = {};
...
// Additional function to calculate daily stats, such as total caffeine, cost, etc.
export function calculateCoffeeStats(
  coffeeConsumptionHistory: CoffeeConsumptionHistory
) {
  const dailyStats: {
  ...
  for (const [timestamp, coffee] of Object.entries(coffeeConsumptionHistory)) {
    ...
    const cost = parseFloat(coffee.cost.toString());
